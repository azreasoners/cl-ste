...
"""
Write a clause below for r in {0, ..., 198}
-sum(r) v V_{ab + cd = r} predict(a,b,c,d)
"""
...
def write_cnf(path_cnf, path_atom2idx):
    ...
    # define 10^4 atoms for predict(n1,n2,n3,n4)
    for n1 in range(10):
        for n2 in range(10):
            for n3 in range(10):
                for n4 in range(10):
                    atom2idx[f'predict({n1},{n2},{n3},{n4})'] = idx
                    idx += 1

    # define 199 atoms for sum
    for r in range(199):
    ...
    # add 199 rules of the form -sum(r) v Disjunction_{d1,d2,d3,d4 | d1d2+d3d4=r} predict(d1,d2,d3,d4)
    for r in range(199):
        atom = atom2idx[f'sum({r})']
        atoms = [str(atom2idx[f'predict({d1},{d2},{d3},{d4})']) \
                    for d1 in range(10) 
                    for d2 in range(10) 
                    for d3 in range(10) 
                    for d4 in range(10) 
...
class Net(nn.Module):
    def __init__(self, reg, lr, bn, device, b):
        ...
        self.C, self.atom2idx = read_cnf('./cnf/mnistAdd2.cnf', './cnf/mnistAdd2.atom2idx')
    ...
    def do_gradient_update(self, data, debug, device, hyper, b):
        """
        @param data: a tuple (images, label) where images is of shape (4, 1, 28, 28) denoting 4 digit images
                                                   and label is an integer in {0, ..., 199}
    ...
    def vg_gen(self, x_label, label, device):
        ...
        # where the last 199 atoms are sum/1
        g[:,-199:] = F.one_hot(label, num_classes=199)

        # 2. construct v from NN output and g
        # assume batchSize is 1
        x_label = x_label.view(4,10)
        x_label = torch.nn.functional.softmax(x_label, dim=-1)

        # 2.1 the first 10^4 atoms in v represent predict/4
        v[:,:10000] = B(torch.cartesian_prod(x_label[0,:], x_label[1,:], x_label[2,:], x_label[3,:]).prod(dim=-1))

        # 2.3 overwrite v with g
        v = g + v * (g==0).int()

        # 3. simplify C by filtering out the unrelated clauses in C 
        mask = torch.matmul(g, self.C.T) == -1 # (batchSize, m); m=199
        C = self.C.unsqueeze(0).repeat(batch_size,1,1) # (batchSize, m, n)
        C = C[mask] # (batchSize, m', n); dimention m' will be removed if m' = 1
...
class MNIST_Addition(Dataset):
    ...
    def __getitem__(self, index):
        i1, i2, i3, i4, r = self.data[index]
        return torch.cat((self.dataset[i1][0], self.dataset[i2][0], self.dataset[i3][0], self.dataset[i4][0]), 0).unsqueeze(1), r

transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081, ))])
# used for training
trainDataset = MNIST_Addition(torchvision.datasets.MNIST(root='./data/', train=True, download=True, transform=transform), './data/mnistAdd2_train.txt')
