...
"""
CNF for b_p(x)+iSTE
Write 4 clauses below for all d in {0,...,9}
-in(d, true) v digit(0, d) v digit(1, d) v digit(2, d)
...
def write_cnf(path_cnf, path_atom2idx):
    ...
    numClauses = 0
    cnf2 = ''
    # (40 domain specific clauses)
    for d in range(10):
        atoms = [atom2idx[atom] for atom in [f'in({d},1)', f'digit(0,{d})', f'digit(1,{d})', f'digit(2,{d})']]
        cnf2 += f'{atoms[1]} {atoms[2]} {atoms[3]} -{atoms[0]} 0 \n'
        numClauses += 1
        for i in range(3):
            atoms = [atom2idx[atom] for atom in [f'in({d},0)', f'digit({i},{d})']]
            cnf2 += f'-{atoms[1]} -{atoms[0]} 0\n'
            numClauses += 1
...
Write clauses for UEC rule for img in {0,1,2} and i,j in {0,...,9} such that i<j
...
def write_cnf_UEC(path_cnf, path_atom2idx):
    ...
    # (3 clauses for EC) add 3 rules of the form digit(img,0) v ... v digit(img,9)
    for img in ('0', '1', '2'):
    ...
    # (3*45=135 clauses for UC) add a rule "-digit(img,i) v -digit(img,j)" for img in {0,1,2} and i,j in {0,...,9} such that i<j
    for img in ('0', '1', '2'):
...
class Net(nn.Module):
    def __init__(self, reg, lr, bn, device, b):
        ...
        if b == 'B':
            self.C, self.atom2idx = read_cnf('./cnf/member3.cnf', './cnf/member3.atom2idx')
        else:
            self.C, self.atom2idx = read_cnf_UEC('./cnf/member3_UEC.cnf', './cnf/member3_UEC.atom2idx')
    ...
    def forward(self, x):
        """
        @param x: NN input of shape (batch_size, 3, 1, 28, 28)
        """
        # each data instance in a batch has 3 images; we group all images in a batch together
        x = x.view(-1, 1, 28, 28) # (batch_size*3, 1, 28, 28)
        x = self.encoder(x)
        x = x.view(-1, 16 * 4 * 4)
        x = self.classifier(x)
        return x # (batch_size*3, 10)

    def do_gradient_update(self, data, debug, device, hyper, b):
        """
        @param data: a tuple (images, digit, label) where 
                     images is a tensor of shape (3, 1, 28, 28) denoting 3 digit images
    ...
    def vg_gen(self, output, digit, label, b, device):
        ...
        if b == 'B':
            ...
            output = F.softmax(output, dim=-1).view(batch_size, 30) # (batch_size, 30)

            # 3. construct v from NN output and g
            # 3.1 first 30 atoms in v represent digit/2
            v[:,:30] = B(output)
        ...
        output = self.mapping[b](output).view(batch_size, 30) # (batch_size, 30)
        # 2.2 first 30 atoms in v represent digit/2
        v[:,:30] = output
    ...
    def vg_gen_general(self, output, digit, label, device):
        ...
        # 4. simplify C by filtering out the unrelated clauses in C
        mask = (torch.matmul(g, self.C.T) == -1).unsqueeze(-1) # (batch_size, m, 1); m=40
